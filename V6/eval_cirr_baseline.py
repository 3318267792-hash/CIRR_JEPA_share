...
def main():
    ...
    max_gallery_batches = None
    max_query_batches = None

    # ===== 新增：评估 MLP 版本时启用 query mapper =====
    encoder = QwenEmbeddingEncoder(device=device, use_query_mapper=True).to(device)

    checkpoint_path = "checkpoints/last_qwen_mlp.pt"
    if os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location=device)
        encoder.load_state_dict(checkpoint["query_encoder"])
    ...
    with torch.no_grad():
        for query_batch_idx, batch in enumerate(query_dataloader):
            ...
            # ===== 新增：query 侧显式走 mapper =====
            pred_embs = encoder.encode_queries(ref_images, captions, use_mapper=True)
    ...
    eval_log_path = "checkpoints/eval_log_qwen_mlp.csv"
    ...
    print("\n===== CIRR Qwen MLP Evaluation Results =====")
